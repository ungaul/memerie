  ...
  $(document).ready(function() {
    ...
    $("#upload-button").on("click", function() {
      ...
      fileInput.on("change", function(e) {
        ...
        // Create the pending upload row at the end of the file list
        var pendingRow = $(`
        ...
        inputField.focus();
        // Automatically select only the filename part (without extension) in the input
    ...
    $(document).on("click", ".pending-check", function(e) {
      ...
      // If the extension was removed, put it back.
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.status === 'success') {
              ...
              var formattedDate = uploadedFile.modifiedTime ? new Date(uploadedFile.modifiedTime).toLocaleString() : '-';
              ...
              // Insert newRow in alphabetical order among files, below any folders.
              var container = $('#drive-rows');
              var inserted = false;
              // Get all rows that are files (i.e., not pending, not back, and not folders)
              ...
              if (fileRows.length > 0) {
                ...
                // If no file rows exist, append after the last folder row (if any) or at the end.
              ...
              pendingRow.remove();
              $("#notification").addClass("enabled");
              setTimeout(() => {
                $("#notification").removeClass("enabled");
              }, 2000);
            }
          },
